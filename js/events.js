document.addEventListener("DOMContentLoaded", function () {
  ...
  // to save the event in the local storage
  ...
  function openModal(day, month, year) {
    ...
    const eventData = eventStorage[key] || {};
  ...
  function updateCalendarEvents() {
    document.querySelectorAll(".day-cell").forEach((cell) => {
      ...
      if (!Array.isArray(eventStorage[key])) {
        eventStorage[key] = eventStorage[key] ? [eventStorage[key]] : [];
      }
  ...
  document.getElementById("eventForm").addEventListener("submit", function (e) {
    ...
    // verify if arledy exist an event in the key
    if (!eventStorage[key]) {
        eventStorage[key] = [];  // initialize the array if it doesn't exist
    }

    // add the event to the array
    eventStorage[key].push({ title, description, time });
